-- S1 (audit 7 Agu 2026): tutup kebocoran data tamu lewat wildcard ILIKE.
--
-- Versi lama mencari booking dengan `reference_code ILIKE p_id LIMIT 1`.
-- Karena `%` dan `_` adalah wildcard ILIKE, satu request publik dengan
-- p_id = '%' mengembalikan invoice lengkap milik tamu acak: nama, email,
-- nomor HP, plus nomor rekening properti. Fungsi ini SECURITY DEFINER dan
-- di-GRANT ke anon, jadi tidak ada lapisan RLS yang menahannya.
--
-- Perbaikan:
--   1. Validasi bentuk kode booking sebelum query (huruf/angka/strip saja).
--   2. Pencocokan persis case-insensitive (upper = upper), bukan pola.
--   3. Tidak ada perubahan pada payload yang dikembalikan — hanya cara
--      booking ditemukan.
...
DECLARE
  ...
  v_ref text;
BEGIN
  v_ref := upper(btrim(COALESCE(p_id, '')));
  ...
  IF v_is_uuid THEN
    v_booking_uuid := v_ref::uuid;
  ELSE
    -- Hanya terima bentuk kode booking yang wajar. Wildcard ('%', '_'),
    -- spasi, dan karakter lain ditolak sebelum menyentuh tabel.
    IF v_ref !~ '^[A-Z0-9-]{3,20}$' THEN
      RETURN NULL;
    END IF;

    SELECT id INTO v_booking_uuid
    FROM public.bookings
    WHERE upper(reference_code) = v_ref
    LIMIT 1;
  END IF;
...
-- Index pendukung pencocokan case-insensitive.
CREATE INDEX IF NOT EXISTS idx_bookings_reference_code_upper
  ON public.bookings (upper(reference_code));
