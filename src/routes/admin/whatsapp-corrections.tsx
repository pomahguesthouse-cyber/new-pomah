...
function WhatsappCorrectionsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return candidates.filter((c) =>
      [c.phone, c.display_name, c.user_message, c.bot_wrong_reply, c.intent, c.agent_key]
        .filter(Boolean)
    ...
    <div className="flex h-screen min-h-0 flex-col bg-stone-100">
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-card px-3 py-2 sm:gap-4 sm:px-5 sm:py-3">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" disabled={backfillMut.isPending} onClick={() => backfillMut.mutate()}>
            {backfillMut.isPending && <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" />}
            Backfill Embedding
          </Button>
        </div>
      </header>
        ...
        <section className="flex min-h-0 flex-col rounded-xl border bg-card shadow-sm">
          <div className="border-b p-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h2 className="text-sm font-semibold">Kandidat balasan bot</h2>
                <p className="text-xs text-muted-foreground">
                  Pilih balasan yang salah lalu tulis jawaban ideal.
                </p>
              </div>
              <div className="relative w-80">
                ...
                <Input
                  ...
                  placeholder="Cari pesan, nomor, nama..."
                />
              </div>
            </div>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto p-3">
            {isFetching && filtered.length === 0 ? (
              ...
              <div className="space-y-3">
                {filtered.map((c) => (
                  <button
                    key={c.wrong_reply_message_id}
                    onClick={() => open(c)}
                    className={cn(
                      "block w-full rounded-lg border bg-background p-3 text-left hover:border-teal-400",
                      selected?.wrong_reply_message_id === c.wrong_reply_message_id && "border-teal-500 bg-teal-50",
                    )}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-semibold">{c.display_name || c.phone}</p>
                        <p className="font-mono text-[10px] text-muted-foreground">{c.phone}</p>
                      </div>
                      <div className="flex gap-1">
                        {c.intent && <Badge variant="outline" className="text-[10px]">{c.intent}</Badge>}
                        {c.agent_key && (
                    ...
                    <div className="mt-3 grid gap-2 lg:grid-cols-2">
                      <div className="rounded-md border bg-white p-2">
                        ...
                        <p className="mt-1 line-clamp-4 text-xs">{c.user_message}</p>
                        <p className="mt-1 text-[10px] text-muted-foreground">{formatTimeID(c.user_sent_at)}</p>
                      </div>
                      <div className="rounded-md border border-rose-200 bg-rose-50 p-2">
                        <p className="text-[10px] font-semibold uppercase text-rose-700">Balasan bot</p>
                        <p className="mt-1 line-clamp-4 text-xs">{c.bot_wrong_reply}</p>
                        <p className="mt-1 text-[10px] text-muted-foreground">{formatTimeID(c.bot_sent_at)}</p>
                      </div>
                    </div>
                  </button>
                ))}
          ...
          <section className="rounded-xl border bg-card p-4 shadow-sm">
            ...
            {!selected ? (
              <p className="mt-3 rounded-lg border border-dashed p-4 text-xs text-muted-foreground">
                Pilih kandidat di kiri.
              </p>
            ) : (
              <div className="mt-3 space-y-3">
                <div className="rounded-md bg-muted/40 p-2 text-xs">
                  <p className="font-semibold">{selected.display_name || selected.phone}</p>
                  <p className="mt-1 line-clamp-2 text-muted-foreground">{selected.user_message}</p>
                </div>
                <label className="block space-y-1">
                  <span className="text-xs font-medium">Jawaban ideal</span>
                  <Textarea
                    rows={7}
                    value={idealReply}
                    onChange={(e) => setIdealReply(e.target.value)}
                    placeholder="Tulis jawaban yang seharusnya dikirim bot..."
                    className="text-xs"
                  />
                </label>
                <div className="grid grid-cols-2 gap-2">
                  <SelectField label="Intent" value={correctIntent} onChange={setCorrectIntent} options={INTENTS} />
                  <SelectField label="Agent" value={correctAgent} onChange={setCorrectAgent} options={AGENTS} />
                  <SelectField label="Error" value={errorType} onChange={setErrorType} options={ERRORS} />
                </div>
                <label className="block space-y-1">
                ...
                <Button className="w-full" disabled={!idealReply.trim() || saveMut.isPending} onClick={() => saveMut.mutate()}>
                  {saveMut.isPending ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-1.5 h-4 w-4" />}
                  Simpan & Training
                </Button>
              </div>
            )}
          </section>
          <section className="min-h-0 flex-1 overflow-y-auto rounded-xl border bg-card p-3 shadow-sm">
            <h2 className="text-sm font-semibold">Koreksi tersimpan</h2>
            <div className="mt-3 space-y-2">
              {corrections.map((c) => (
                <div key={c.id} className="rounded-lg border p-2 text-xs">
                  <div className="flex justify-between gap-2">
                    <Badge variant="outline" className="text-[9px]">{c.status}</Badge>
                    <span className="text-[10px] text-muted-foreground">{formatRelativeDateID(c.created_at)}</span>
                  </div>
                  <p className="mt-2 line-clamp-2 font-medium">{c.user_message}</p>
                  <p className="mt-1 line-clamp-2 text-muted-foreground">Ideal: {c.ideal_reply}</p>
                  <div className="mt-2 flex gap-1">
                    {c.correct_intent && <Badge variant="secondary" className="text-[9px]">{c.correct_intent}</Badge>}
                    {c.correct_agent && <Badge variant="secondary" className="text-[9px]">{c.correct_agent}</Badge>}
                    {c.embedding_updated_at ? (
                      <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 text-[9px]">embedded</Badge>
                    ) : (
                      <Badge variant="outline" className="text-[9px] text-amber-700">no embedding</Badge>
                    )}
                  </div>
                </div>
              ))}
