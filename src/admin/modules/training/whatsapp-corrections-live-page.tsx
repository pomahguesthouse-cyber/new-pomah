...
import { Loader2, MessageCircle, RefreshCw, Save, Search, Trash2 } from "lucide-react";
...
export function WhatsappCorrectionsPage() {
  ...
  const saveSessionMut = useMutation({
    ...
    onSuccess: () => { toast.success("Percakapan live disimpan sebagai training context."); setRightTab("saved"); qc.invalidateQueries({ queryKey: ["wa-correction-sessions"] }); },
  ...
  return (
    <div className="flex h-screen min-h-0 bg-[#f0f2f5] text-[#111b21]">
      <aside className="flex min-h-0 w-[380px] min-w-[320px] max-w-[420px] flex-col border-r bg-white">
        <div className="flex h-[58px] items-center justify-between bg-[#008069] px-4 text-white">
          <div><div className="font-semibold">WA Correction</div><div className="text-[10px] text-white/80">Live WPPConnect VPS</div></div>
          <Button size="sm" variant="outline" className="h-7 rounded-full border-white/45 bg-white/10 px-3 text-xs text-white hover:bg-white/20 hover:text-white" disabled={loadingThreads} onClick={() => refetchChats()}>{loadingThreads ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <RefreshCw className="mr-1 h-3 w-3" />}Live</Button>
        </div>
        <div className="border-b px-3 py-2"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#667781]" /><Input value={search} onChange={(e) => setSearch(e.target.value)} className="h-9 rounded-full border-0 bg-[#f0f2f5] pl-9 text-sm shadow-none focus-visible:ring-0" placeholder="Cari live chat..." /></div></div>
        <div className="min-h-0 flex-1 overflow-y-auto">
          {chatsError ? <div className="p-4 text-sm text-red-600">{(chatsError as Error).message}</div> : loadingThreads && filteredThreads.length === 0 ? <p className="p-4 text-sm text-[#667781]">Memuat live chat...</p> : filteredThreads.length === 0 ? <div className="p-8 text-center text-sm text-[#667781]">Belum ada percakapan live.</div> : filteredThreads.map((t) => <ThreadListItem key={t.id} thread={t} active={selectedThread?.id === t.id} onClick={() => setSelectedThreadId(t.id)} />)}
        </div>
      </aside>
      <section className="flex min-w-0 flex-1 flex-col">
        {selectedThread ? <><ThreadHeader thread={selectedThread} loading={loadingMessages} onRefresh={() => refetchMessages()} /><div className="min-h-0 flex-1 overflow-y-auto bg-[#efeae2] px-[clamp(1.25rem,4vw,5rem)] py-5">{loadingMessages ? <p className="rounded-lg bg-white/80 px-3 py-2 text-sm text-[#667781] shadow-sm">Mengambil pesan langsung dari VPS...</p> : <MessageTimeline messages={trainingMessages} latestRef={latestRef} editingMessageId={editingMessageId} setEditingMessageId={setEditingMessageId} draftBody={draftBody} setDraftBody={setDraftBody} editedBodies={editedBodies} setEditedBodies={setEditedBodies} />}</div></> : <div className="flex flex-1 items-center justify-center bg-[#f0f2f5] p-8 text-center text-sm text-[#667781]">Pilih percakapan live.</div>}
      </section>
      <aside className="flex min-h-0 w-[380px] flex-col border-l bg-white">
        <div className="border-b bg-[#f0f2f5] p-3"><div className="grid grid-cols-2 gap-1 rounded-full bg-white p-1 shadow-sm"><button onClick={() => setRightTab("context")} className={cn("rounded-full px-3 py-2 text-xs font-semibold", rightTab === "context" ? "bg-[#008069] text-white" : "text-[#667781]")}>Konteks</button><button onClick={() => setRightTab("saved")} className={cn("rounded-full px-3 py-2 text-xs font-semibold", rightTab === "saved" ? "bg-[#008069] text-white" : "text-[#667781]")}>Tersimpan</button></div></div>
...
function ThreadListItem({ thread, active, onClick }: { thread: ThreadRow; active: boolean; onClick: () => void }) {
  return <button onClick={onClick} className={cn("flex w-full items-center gap-3 border-b px-4 py-3 text-left hover:bg-[#f5f6f6]", active && "bg-[#f0f2f5]")}><AvatarCircle thread={thread} /><div className="min-w-0 flex-1"><div className="flex items-baseline justify-between gap-2"><p className="truncate text-[13px] font-semibold">{formatWaPhone(primaryPhone(thread))}</p>{thread.last_message_at && <span className="shrink-0 text-[10px] text-[#667781]">{formatRelativeDateID(thread.last_message_at)}</span>}</div><p className="mt-0.5 truncate text-xs text-[#667781]">{displayName(thread)}{thread.identity_type === "lid" ? " · LID live" : ""}</p><p className="mt-0.5 truncate text-xs text-[#667781]">{thread.last_message_preview || "-"}</p></div></button>;
}
function ThreadHeader({ thread, loading, onRefresh }: { thread: ThreadRow; loading: boolean; onRefresh: () => void }) {
  return <header className="flex h-[58px] items-center gap-3 border-b bg-white px-5"><AvatarCircle thread={thread} /><div className="min-w-0 flex-1"><p className="truncate text-[15px] font-semibold">{formatWaPhone(primaryPhone(thread))}</p><p className="truncate text-xs text-[#667781]">{displayName(thread)} · live VPS{thread.external_chat_id ? ` · ${thread.external_chat_id}` : ""}</p></div><Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100">Live</Badge><Button size="sm" variant="outline" className="h-8 rounded-full px-3 text-xs" disabled={loading} onClick={onRefresh}>{loading ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="mr-1 h-3.5 w-3.5" />}Refresh</Button></header>;
}
function AvatarCircle({ thread }: { thread: ThreadRow | null }) { return <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#dfe5e7] text-sm font-semibold text-[#54656f]">{thread ? avatarText(thread) : <MessageCircle className="h-5 w-5" />}</div>; }
...
function ChatBubble(props: { message: MessageRow; latestRef: RefObject<HTMLDivElement | null> | null; editingMessageId: string | null; setEditingMessageId: (id: string | null) => void; draftBody: string; setDraftBody: (value: string) => void; editedBodies: Record<string, string>; setEditedBodies: Dispatch<SetStateAction<Record<string, string>>> }) {
  ...
  return <div ref={props.latestRef ?? undefined} className={cn("mb-1 flex", out ? "justify-end" : "justify-start")}><div className={cn("w-fit max-w-[min(620px,56%)] rounded-lg px-2 py-1 text-[14px] shadow", out ? "bg-[#d9fdd3]" : "bg-white")}>{editing ? <Textarea rows={5} value={props.draftBody} onChange={(e) => props.setDraftBody(e.target.value)} className="min-w-[420px] bg-white text-sm" /> : <p className="whitespace-pre-wrap break-words">{shown}</p>}<div className="mt-1 flex items-center justify-end gap-1 text-[10px] text-[#667781]">{props.editedBodies[m.id] && <Badge className="h-4 bg-[#e7fce3] px-1.5 text-[9px] text-[#008069]">edited</Badge>}<Badge className="h-4 bg-sky-100 px-1.5 text-[9px] text-sky-700">live</Badge><span>{formatTimeID(m.sent_at)}</span></div>{out && <div className="mt-2 flex justify-end gap-1">{editing ? <Button size="sm" className="h-7 rounded-full bg-[#008069] px-2 text-[11px]" onClick={() => { props.setEditedBodies((prev) => ({ ...prev, [m.id]: props.draftBody })); props.setEditingMessageId(null); }}>Terapkan</Button> : <Button size="sm" variant="ghost" className="h-7 rounded-full px-2 text-[11px] text-[#667781]" onClick={() => { props.setEditingMessageId(m.id); props.setDraftBody(shown); }}>Edit Koreksi</Button>}</div>}</div></div>;
}
function RightContext(props: { summary: string; setSummary: (value: string) => void; saveSessionMut: SimpleMutation; trainingMessagesCount: number; selectedThread: ThreadRow | null; mapPhone: string; setMapPhone: (value: string) => void; mapIdentityMut: SimpleMutation }) {
  ...
  return <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4"><section className="rounded-lg border bg-white p-3 shadow-sm"><h2 className="text-sm font-semibold">Training Context Live</h2><p className="mt-1 text-xs text-[#667781]">Pesan dibaca langsung dari WPPConnect VPS; hasil koreksi disimpan ke Supabase training.</p><Textarea rows={10} className="mt-3 text-xs" value={props.summary} onChange={(e) => props.setSummary(e.target.value)} /><Button className="mt-3 w-full rounded-full bg-[#008069] hover:bg-[#00695c]" disabled={!props.trainingMessagesCount || props.saveSessionMut.isPending} onClick={() => props.saveSessionMut.mutate()}>{props.saveSessionMut.isPending ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Save className="mr-1 h-4 w-4" />}Simpan Percakapan Live</Button></section><section className="rounded-lg border bg-white p-3 text-xs text-[#667781]"><h2 className="text-sm font-semibold text-[#111b21]">Identitas Live</h2><p className="mt-2">Nomor: {formatWaPhone(primaryPhone(props.selectedThread))}</p><p>ChatId: {props.selectedThread?.external_chat_id || "-"}</p><p>Source: {props.selectedThread?.source || "wppconnect_live"}</p>{lid && !isPublicPhone(primaryPhone(props.selectedThread)) && <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3"><p className="font-semibold text-amber-800">Map LID ke Nomor WA</p><p className="mt-1 text-[11px] text-amber-700">LID: {lid}</p><Input className="mt-2 h-8 bg-white text-xs" placeholder="6281234567890" value={props.mapPhone} onChange={(e) => props.setMapPhone(e.target.value)} /><Button size="sm" className="mt-2 h-8 w-full rounded-full bg-[#008069] text-xs hover:bg-[#00695c]" disabled={props.mapIdentityMut.isPending || !props.mapPhone.trim()} onClick={() => props.mapIdentityMut.mutate()}>{props.mapIdentityMut.isPending ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : null}Simpan Mapping</Button></div>}</section></div>;
}
function SavedPanel({ sessions, deleteSessionMut }: { sessions: SessionRow[]; deleteSessionMut: SimpleMutation }) {
  return <div className="min-h-0 flex-1 overflow-y-auto p-4"><div className="mb-3"><h2 className="text-sm font-semibold">Percakapan tersimpan</h2><p className="text-xs text-[#667781]">Daftar full conversation training yang sudah disimpan.</p></div><div className="space-y-3">{sessions.length === 0 ? <div className="rounded-lg border border-dashed p-4 text-center text-xs text-[#667781]">Belum ada percakapan tersimpan.</div> : sessions.map((s) => <div key={s.id} className="rounded-lg border bg-white p-3 text-xs shadow-sm"><div className="flex items-center justify-between gap-2"><Badge variant="outline" className="text-[9px]">{s.status}</Badge><div className="flex items-center gap-1">{s.embedding_updated_at ? <Badge className="bg-emerald-100 text-[9px] text-emerald-700">embedded</Badge> : <Badge variant="outline" className="text-[9px]">no embedding</Badge>}<Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" disabled={deleteSessionMut.isPending} onClick={() => { if (window.confirm("Hapus percakapan tersimpan ini dari training?")) deleteSessionMut.mutate(s.id); }}><Trash2 className="h-3.5 w-3.5" /></Button></div></div><p className="mt-3 font-semibold leading-snug">{s.title || "Percakapan"}</p><p className="mt-1 whitespace-pre-wrap text-[#667781]">{s.conversation_summary || "-"}</p><p className="mt-2 text-[10px] text-[#667781]">{formatRelativeDateID(s.created_at)}</p></div>)}</div></div>;
}
