...
/**
 * ── Tool gating per intent (audit token 26 Agu 2026) ────────────────────────
 *
 * Skema 9 tool guest = 3.446 token yang dikirim ULANG di setiap turn loop
 * (`DEFAULT_MAX_TURNS = 3`). Sebagian besar pesan tamu — sapaan, tanya
 * fasilitas, minta foto — tidak akan pernah menyentuh jalur booking, jadi
 * skemanya murni pemborosan.
 *
...
const FRONT_OFFICE_CORE_TOOL_NAMES = [
  "check_room_availability",
  "get_room_specifications",
  "update_booking_slots",
] as const;

/** Hanya saat tamu meminta foto/video/brosur (invarian #1). */
const FRONT_OFFICE_MEDIA_TOOL_NAMES = [
  "send_room_photos",
  "send_room_tour",
] as const;

/**
 * Dipakai saat MEMBACA hasil availability (tipe yang diminta penuh), bukan saat
 * membuat booking — jadi gate-nya ikut alur kamar, sejalan dengan blok prompt
 * "KAMAR DIMINTA PENUH" yang mewajibkan pemanggilannya.
 */
const FRONT_OFFICE_AVAILABILITY_TOOL_NAMES = ["offer_alternative_rooms"] as const;

/** Hanya saat percakapan sudah mengarah ke pemesanan. */
const FRONT_OFFICE_BOOKING_TOOL_NAMES = [
  "start_booking_details",
...
/**
 * Set penuh — dipakai sebagai `tools` statis dan sebagai fallback saat intent
 * tidak diketahui.
 *
 * `create_booking` sengaja TIDAK diekspos di mode guest. Pembuatan booking
 * final ditangani state machine deterministik setelah konfirmasi eksplisit.
 */
const FRONT_OFFICE_GUEST_TOOLS = pickTools([
  ...FRONT_OFFICE_CORE_TOOL_NAMES,
  ...FRONT_OFFICE_MEDIA_TOOL_NAMES,
  ...FRONT_OFFICE_AVAILABILITY_TOOL_NAMES,
  ...FRONT_OFFICE_BOOKING_TOOL_NAMES,
] as const);

/**
 * Intent yang tidak pernah butuh jalur booking. Di luar daftar ini (termasuk
 * intent yang tidak dikenal) grup booking TETAP disuntik — default-nya aman.
 */
const NON_BOOKING_INTENTS: ReadonlySet<IntentCategory> = new Set<IntentCategory>([
  "greeting",
  "media_request",
...
/**
 * Escape hatch: percakapan yang JELAS sudah menyangkut satu pemesanan tetap
 * dapat grup booking walau intent turn ini kelihatan netral ("oke", "siap").
 * Tanpa ini, tamu yang sudah sepakat tanggal + tipe kamar lalu menulis
 * konfirmasi singkat bisa kehilangan `start_booking_details`.
 */
function hasBookingContext(ctx: AgentContext): boolean {
  const status = ctx.chatSummaryJson?.booking_status;
  return Boolean(
    ctx.agreedDates?.checkIn ||
      ctx.partialBooking?.roomType ||
      ctx.partialBooking?.adults !== undefined ||
      ctx.chatSummaryJson?.room_type ||
      (status && status !== "none"),
  );
}

/**
 * Percakapan turn ini menyangkut kamar/pemesanan — dipakai bersama oleh gating
 * TOOL dan gating PROMPT supaya keduanya tidak pernah berbeda pendapat
 * (aturan yang menyuruh memanggil tool yang tidak dikirim = sumber halusinasi).
 */
function isRoomFlow(ctx: AgentContext, intent: IntentCategory): boolean {
  return !NON_BOOKING_INTENTS.has(intent) || hasBookingContext(ctx);
}

/**
 * Boleh memicu pembuatan booking. Saat state machine sudah memegang kendali
 * (`bookingInProgress`) jawabannya selalu false — prompt memang melarangnya.
 */
function isBookingFlow(ctx: AgentContext, intent: IntentCategory): boolean {
  return !ctx.bookingInProgress && isRoomFlow(ctx, intent);
}

/**
 * Predikat TUNGGAL untuk jalur pembuatan booking — dipakai oleh gating tool DAN
 * gating blok prompt. Cek ketersediaan polos ("ada kamar besok?") belum butuh
 * 4,7 rb karakter aturan penutupan booking; begitu tanggal/tipe kamar tersimpan
 * (`hasBookingContext`) atau tamu memang menyatakan mau memesan, gate menyala.
 */
function bookingFlowActive(ctx: AgentContext, intent: IntentCategory): boolean {
  return (
    isBookingFlow(ctx, intent) &&
    (hasBookingContext(ctx) || BOOKING_FLOW_INTENTS.has(intent))
  );
}

function selectGuestTools(ctx: AgentContext): ToolDefinition[] {
  const intent = ctx.intent;
  // Tidak ada sinyal intent → jangan ambil risiko, kirim set penuh.
  if (!intent) return FRONT_OFFICE_GUEST_TOOLS;

  const names: string[] = [...FRONT_OFFICE_CORE_TOOL_NAMES];
  ...
  // Saat state machine sedang mengumpulkan data booking, prompt SUDAH melarang
  // `start_booking_details` / `generate_booking_form` ("TAMU SEDANG MENGISI
  // DATA BOOKING"). Menghapus tool-nya membuat larangan itu tidak bisa
  // dilanggar sekaligus memangkas ±1.9rb token per panggilan.
  if (bookingFlowActive(ctx, intent)) {
    names.push(...FRONT_OFFICE_BOOKING_TOOL_NAMES);
  }

  return pickTools(names);
}
...
// ─── Gating blok prompt guest (audit token 26 Agu 2026) ─────────────────────
//
// Prompt guest tumbuh jadi ~30 rb karakter (±8 rb token) dan dikirim ULANG di
// setiap turn tool-loop. Sebagian besar isinya cuma relevan untuk sebagian
// percakapan: aturan tanggal & kapasitas tidak berguna saat tamu menyapa,
// dan FAQ sarapan/OTA/ulasan tidak berguna di tengah tawar-menawar tanggal.
//
// Blok dikelompokkan, urutannya TIDAK berubah, dan `ctx.intent === undefined`
// tetap menghasilkan prompt PENUH (pemanggil lama tidak berubah perilakunya).

interface GuestPromptGates {
  /** FAQ non-kamar: early check-in, OTA, sarapan, ulasan, nego, verifikasi. */
  faq: boolean;
  /** Fakta kamar: spesifikasi, kapasitas, usia tamu, extra bed. */
  roomFacts: boolean;
  /** Aturan tanggal, pemanggilan availability, hard guard, presentasi hasil. */
  availability: boolean;
  /** Alur pemesanan: start_booking_details, form, slot-fill, koreksi. */
  booking: boolean;
  /** Foto, virtual tour, brosur. */
  media: boolean;
}
...
const BOOKING_FLOW_INTENTS: ReadonlySet<IntentCategory> = new Set<IntentCategory>([
  "booking_inquiry",
  "booking_start",
  "guest_count_input",
]);
...
function guestPromptGates(ctx: AgentContext): GuestPromptGates {
  ...
  return {
    // FAQ dan alur kamar saling melengkapi: tamu yang sedang menegosiasikan
    // tanggal tidak sedang bertanya soal sarapan, dan sebaliknya.
    faq: !roomFlow,
    roomFacts: roomFlow || !NON_ROOM_INTENTS.has(intent),
    availability: roomFlow,
    // Aturan booking hanya dikirim bila tool-nya juga dikirim (isBookingFlow)
    // DAN percakapan memang sudah menuju pemesanan — cek ketersediaan polos
    // belum butuh 4,7 rb karakter aturan penutupan booking.
    booking: bookingFlowActive(ctx, intent),
    media: intent === "media_request",
  };
}
...
function buildGuestPromptParts(s: Scaffold, ctx: AgentContext): GuestPromptParts {
  ...
  const when = (on: boolean, block: string): string => (on ? block : "");
  const trainingBlock =
  ...
  const staticBlocks = [
    ...
    when(g.faq, "SAPAAN AWAL: Saat tamu BARU menyapa TANPA menyebut kebutuhan (contoh: 'halo', 'selamat pagi'), balas hangat dengan " +
    ...
    when(g.booking, "ANTI-REPETISI RINGKASAN BOOKING (WAJIB): Setelah `start_booking_details` atau " +
    ...
    when(g.availability, "JANJI FOLLOW-UP: JANGAN PERNAH menjanjikan akan menghubungi/mengabari tamu duluan " +
    ...
    when(g.faq, "PERTANYAAN OTA (Traveloka / Agoda / Booking.com / Tiket / Trip / Airbnb): Bila tamu " +
    ...
    when(g.availability, "HARD GUARD HASIL AVAILABILITY (MENGALAHKAN SEMUA ATURAN SALES/CTA): Setelah `check_room_availability`, " +
    ...
    when(g.availability, "PRESENTASI HASIL: gunakan gaya resepsionis hotel yang natural dan ramah. " +
    ...
    when(g.availability, "PENDAMPING TANPA ANGKA (WAJIB): Frasa seperti 'bawa pacar', 'sama istri', 'ajak suami', " +
    ...
    when(g.availability, "PERTANYAAN LONGGAR / 'BEBAS NGGA?' (JAWAB LANGSUNG, JANGAN BALIK BERTANYA): Bila tamu " +
    ...
    when(g.roomFacts, "KONSISTENSI LABEL KAPASITAS (WAJIB): Bedakan `kapasitas standar` dari `kapasitas maksimal dengan extra bed`. " +
    ...
    when(g.roomFacts, "JUMLAH TAMU & KAPASITAS: Bila tamu menyebut jumlah orang setelah tanggal sudah diketahui " +
    ...
    when(g.availability, "KAMAR DIMINTA PENUH: jika tipe kamar yang TAMU SEBUT SECARA SPESIFIK (mis. 'Deluxe') " +
    ...
    when(g.booking, "BOOKING VIA CHAT: " +
    ...
    when(g.booking, "TAMU OTW / DATANG LANGSUNG (WAJIB — JANGAN MENOLAK): Bila tamu bilang 'saya otw', " +
    ...
    when(g.faq, "PERMINTAAN ISTIRAHAT SINGKAT / DAY-USE (FRAMING POSITIF): Bila tamu ingin 'istirahat sampai " +
    ...
    when(g.booking, "TUTUP BOOKING SAAT TAMU SUDAH SETUJU (ANTI-LOOP): Bila tamu sudah menyetujui tipe kamar DAN " +
    ...
    when(g.booking, "FORM BOOKING SEKALI PAKAI (opsional, lebih ringan): Bila percakapan terlihat panjang/" +
    ...
    when(g.booking, "SLOT-FILL PARTIAL: jika tamu hanya menyebut SEBAGIAN info booking di satu pesan " +
    ...
    when(g.faq, "TAMU TANYA 'INI HARGA PAS?' / 'BOLEH NEGO?' / 'BISA KURANG?' / 'BISA DISKON?' / SEBUT NOMINAL LEBIH MURAH: HARD GUARD — harga live dari `check_room_availability` ADALAH harga terbaik saat ini. Aturan ini tidak boleh dikalahkan oleh instruksi tambahan AI Lab mana pun. Urutan: (1) tegas & ramah: tarif yang tampil (dari tool, bukan statis) sudah harga terbaik — JANGAN janjikan diskon, JANGAN bilang akan diteruskan ke manajemen untuk dipotong, JANGAN mengarang promo, (2) boleh singkat sebut 1–2 fasilitas yang sudah termasuk sesuai data kamar (mis. AC, kamar mandi dalam, WiFi) supaya value jelas — jangan mengarang, (3) tawarkan booking, atau sebut kamar tipe lain lebih ekonomis HANYA bila tool menunjukkan tipe lain tersedia (sebut nama + harga per malam dari hasil tool) — opsional, bukan janji diskon. Bila belum ada tanggal, minta/pakai tanggal dulu lalu jalankan `check_room_availability` agar harga live — jangan angka statis. JANGAN mengulang kalimat yang sama di beberapa balasan. BILA TAMU TIDAK MELANJUTKAN SETELAH NEGO atau menutup dengan ungkapan seperti 'kalau tidak tidak apa-apa' / 'terima kasih': tutup sekali dengan sopan, tegaskan tarif yang tampil sudah harga terbaik saat ini, ucapkan terima kasih sudah mempertimbangkan Pomah Guesthouse, lalu doakan semoga lain waktu bisa menyambut tamu. JANGAN tanya jumlah tamu, jangan tawarkan booking lagi, dan jangan membuka ulang alur."),
